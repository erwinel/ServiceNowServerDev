...
var cu = new x_44813_sec_clsif.ClassificationUtil();
var allClassifications = cu.getAllClassifications();
var expectedClassifications = [
...
if (allClassifications.length < expectedClassifications.length) {
    gs.error("x_44813_sec_clsif.ClassificationUtil.getAllClassifications() failed: Only {0} of expected minimum  {1} results returned.", allClassifications.length, expectedClassifications.length);
}
else if (expectedClassifications.filter(function (c) {
    var arr = allClassifications.filter(function (a) { return a.portion_marking == c.pm; });
    if (arr.length != 1)
        ...
        gs.error("x_44813_sec_clsif.ClassificationUtil.getAllClassifications() failed: Expected name for Portion Marking {0}: {1}; Actual: {2}.", types.serializeToString(c.pm), types.serializeToString(c.name), types.serializeToString(arr[0].name));
    else
        return false;
    return true;
}).length == 0)
    gs.info("x_44813_sec_clsif.ClassificationUtil.getAllClassifications() passed.");
var actual = cu.getDefaultPortionMarking();
...
var cv = new x_44813_sec_clsif.ClassificationValidator();
if (cv.isValid !== false)
...
if (cv.errorMessages.length != 2)
    ...
    var expectedStr = "Classification Name cannot be empty.";
    if (cv.errorMessages[0] !== expectedStr)
        gs.error("new x_44813_sec_clsif.ClassificationValidator().errorMessages[0] failed. Expected: {0}; Actual: {1}", types.serializeToString(expectedStr), types.serializeToString(cv.errorMessages[0]));
    else
    ...
    if (cv.errorMessages[1] !== expectedStr)
        gs.error("new x_44813_sec_clsif.ClassificationValidator().errorMessages[1] failed. Expected: {0}; Actual: {1}", types.serializeToString(expectedStr), types.serializeToString(cv.errorMessages[1]));
    else
        gs.info("update x_44813_sec_clsif.ClassificationValidator().errorMessages[0] passed.");
}
...
if (cv.portionMarking.sourceValue !== " U ")
    gs.error("update x_44813_sec_clsif.ClassificationValidator().portionMarking.sourceValue. Expected: {0}; Actual: {1}", types.serializeToString(" U "), types.serializeToString(cv.portionMarking.sourceValue));
else
...
if (cv.name.sourceValue !== " UNCLASSIFIED")
    gs.error("update x_44813_sec_clsif.ClassificationValidator().name.sourceValue. Expected: {0}; Actual: {1}", types.serializeToString(" UNCLASSIFIED"), types.serializeToString(cv.name.sourceValue));
else
    gs.info("update x_44813_sec_clsif.ClassificationValidator().name.sourceValue passed.");
if (cv.portionMarking.normalizedValue !== "U")
    gs.error("update x_44813_sec_clsif.ClassificationValidator().portionMarking.normalizedValue. Expected: {0}; Actual: {1}", types.serializeToString("U"), types.serializeToString(cv.portionMarking.normalizedValue));
else
...
if (cv.name.normalizedValue !== "UNCLASSIFIED")
    gs.error("update x_44813_sec_clsif.ClassificationValidator().name.normalizedValue. Expected: {0}; Actual: {1}", types.serializeToString("UNCLASSIFIED"), types.serializeToString(cv.name.normalizedValue));
else
...
if (cv.infoMessages.length != 2)
    ...
    var expectedStr = "Portion Marking has been normalized.";
    if (cv.infoMessages[0] !== expectedStr)
        gs.error("update x_44813_sec_clsif.ClassificationValidator().infoMessages[0] failed. Expected: {0}; Actual: {1}", types.serializeToString(expectedStr), types.serializeToString(cv.infoMessages[0]));
    else
    ...
    if (cv.infoMessages[1] !== expectedStr)
        gs.error("update x_44813_sec_clsif.ClassificationValidator().infoMessages[1] failed. Expected: {0}; Actual: {1}", types.serializeToString(expectedStr), types.serializeToString(cv.infoMessages[1]));
    else
        gs.info("update x_44813_sec_clsif.ClassificationValidator().infoMessages[0] passed.");
}
