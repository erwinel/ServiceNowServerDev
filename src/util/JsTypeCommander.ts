namespace x_44813_util {
     ...
     * var tc = new x_44813_util.JsTypeCommander();
     * var deepClone = tc.mapInto([{a: 1, b: 2}, 3, 4, ["Eins", "Svein", "Drei"]], function(current, source, target) => {
     *     if (tc.isArray(current))
     *         return [];
     *     if (tc.isNonArrayObject(current))
     *         return {};
     *     return (tc.isString(current) || tc.isNumber(current) || tc.isBoolean(current) || tc.isObjectOrNil(current)) ? current : current.toString();
     * });
     */
    ...
    class limitingIterator implements MapIntoOptions {
        ...
        maxDepth: number = 32;
        thisObj?: any;
        private _typeCommander: JsTypeCommander = new JsTypeCommander();
    ...
    export class JsTypeCommander {
         ...
         */
        getDefaultLineSeparatorSequence(): string { return patternOptions.newLineSequence; }
