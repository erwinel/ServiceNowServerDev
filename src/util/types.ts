namespace x_44813_util {
    ...
    export class types {
        ...
        static readonly patternDefaults = {
            newLineSequence: "\n",
            regex: {
                onlyWhitespace: /^[\s\r\n]+$/,
                trimStart: /^[\s\r\n]+(\S[\S\s]*)$/,
        ...
        readonly patternOptions = {
            newLineSequence: types.patternDefaults.newLineSequence,
            regex: {
                onlyWhitespace: types.patternDefaults.regex.onlyWhitespace,
                trimStart: types.patternDefaults.regex.trimStart,
                trimEnd: types.patternDefaults.regex.trimEnd,
                lineSeparator: types.patternDefaults.regex.lineSeparator,
                booleanText: types.patternDefaults.regex.booleanText,
                firstLetterLc: types.patternDefaults.regex.firstLetterLc,
                abnormalWhitespace: types.patternDefaults.regex.abnormalWhitespace
            }
        };
        ...
        setPatternOptions(settings?: IJsTypeCommanderRegex): IJsTypeCommanderRegex {
            if (typeof(settings) == "undefined" || settings === null) {
                this.patternOptions.regex.onlyWhitespace = types.patternDefaults.regex.onlyWhitespace;
                this.patternOptions.regex.trimStart = types.patternDefaults.regex.trimStart;
                this.patternOptions.regex.trimEnd = types.patternDefaults.regex.trimEnd;
                this.patternOptions.regex.lineSeparator = types.patternDefaults.regex.lineSeparator;
                this.patternOptions.regex.booleanText = types.patternDefaults.regex.booleanText;
                this.patternOptions.regex.firstLetterLc = types.patternDefaults.regex.firstLetterLc;
                this.patternOptions.regex.abnormalWhitespace = types.patternDefaults.regex.abnormalWhitespace;
            } else if (typeof(settings) == "object") {
                if (this.derivesFrom<RegExp>(settings.onlyWhitespace))
                    this.patternOptions.regex.onlyWhitespace = settings.onlyWhitespace;
                if (this.derivesFrom<RegExp>(settings.trimStart))
                    this.patternOptions.regex.trimStart = settings.trimStart;
                if (this.derivesFrom<RegExp>(settings.trimEnd))
                    this.patternOptions.regex.trimEnd = settings.trimEnd;
                if (this.derivesFrom<RegExp>(settings.lineSeparator))
                    this.patternOptions.regex.lineSeparator = settings.lineSeparator;
                if (this.derivesFrom<RegExp>(settings.booleanText))
                    this.patternOptions.regex.booleanText = settings.booleanText;
                if (this.derivesFrom<RegExp>(settings.firstLetterLc))
                    this.patternOptions.regex.firstLetterLc = settings.firstLetterLc;
                if (this.derivesFrom<RegExp>(settings.abnormalWhitespace))
        ...
        setDefaultLineSeparatorSequence(s?: string): string {
            if (this.isNil(s))
                this.patternOptions.newLineSequence = types.patternDefaults.newLineSequence;
            else {
         ...
         */
        isNull(obj?: TDefined): obj is null|undefined { return typeof(obj) == "object" && obj === null; }
        ...
        indentText(text: string|string[], indent?: string): string {
            ...
            if (i.length == 0)
                i = "\t";
            let t = (Array.isArray(text)) ? text.join(this.patternOptions.newLineSequence) : this.toString(text, "");
            if (i.length == 0 || t.length == 0)
                return t;
        
            return this.splitLines(t).map(s => this.trimEnd(s)).map(s => {
                if (s.length == 0)
                    return s;
                return i + s;
        ...
        indentLines(text: string[]|string, indent?: string): string[] {
            ...
            if (t.length == 0)
                return [t];
            let a = this.splitLines(t).map(s => this.trimEnd(s));
            if (i.length == 0)
                return a;
         ...
         */
        asNumber(obj?: TDefined, defaultValue?: Nullable<number>): Nilable<number>
        {
         ...
         */
        toNumber(obj?: TDefined, defaultValue?: Nullable<number>): number
        {
            let i = this.asNumber(obj, defaultValue);
            if (this.isNumber(i))
                return i;
            return 0;
        }
